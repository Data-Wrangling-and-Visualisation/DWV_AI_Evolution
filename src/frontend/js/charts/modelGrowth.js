...
async function createModelGrowthChart() {
    try {
        ...
        processedData.forEach(model => {
            const date = new Date(model.publishing_date.getFullYear(), model.publishing_date.getMonth(), 1);
        ...
        const monthlyData = Object.values(groupedByMonth).sort((a, b) => a.date - b.date);
        ...
        monthlyData.forEach((month, monthIndex) => {
            // First add this month's counts
            Object.entries(month.categories).forEach(([category, data]) => {
                cumulativeCounts[category][monthIndex] = data.count;
            });
            
            // If not the first month, add previous month's cumulative counts
            if (monthIndex > 0) {
                Object.keys(cumulativeCounts).forEach(category => {
                    cumulativeCounts[category][monthIndex] += cumulativeCounts[category][monthIndex - 1];
                });
            }
        });
        ...
        // Create dropdown for model type selection
        ...
        document.getElementById('modelGrowthContainer').appendChild(dropdownContainer);
        ...
        function filterDatasetsByModelType(type) {
            const datasetsByType = Object.entries(cumulativeCounts)
                .filter(([category, _]) => type === "All" || allCategories[category] === type)
                .map(([category, data]) => ({
                    label: category,
                    data: data,
                    backgroundColor: colorScheme[category] || 'rgba(100, 100, 100, 0.7)',
                    borderColor: colorScheme[category]?.replace('0.7', '1') || 'rgba(100, 100, 100, 1)',
                    borderWidth: 1.5,
                    fill: true,
                    pointRadius: 0, // Hide points to make the chart cleaner
                    tension: 0.1    // Add a small curve to lines
                }));
        ...
        dropdown.addEventListener('change', function() {
            ...
            chart.options.plugins.title.text = selectedType === "All" 
