...
async function createModelPopularityChart() {
  try {
    ...
    console.log(
      "Fetched data for bubble chart via service:",
      data.length,
      "items"
    );
    ...
    if (data && data.length > 0) {
      processedData = data
        .filter(
          (model) => model.publishing_date && model.likes && model.model_size
        )
        .map((model) => ({
          ...model,
    ...
    console.log(
      "Processed data for bubble chart:",
    ...
    const ctx = document
      .getElementById("modelPopularityChart")
      .getContext("2d");
    const maxDownloads = Math.max(
      ...processedData.map((d) => d.downloads || 0)
    );
    const maxModelSize = Math.max(
      ...processedData.map((d) => d.model_size || 0)
    );
    ...
    function getColor(downloads) {
      ...
      return `rgb(${Math.round(
        c1[0] + (c2[0] - c1[0]) * remainder
      )}, ${Math.round(c1[1] + (c2[1] - c1[1]) * remainder)}, ${Math.round(
        c1[2] + (c2[2] - c1[2]) * remainder
      )})`;
    }
    ...
    function getBubbleSize(modelSize) {
      ...
      const normalizedValue =
        Math.cbrt(modelSize || 1) / Math.cbrt(maxModelSize || 1);
    ...
    gradientDiv.style.background =
      "linear-gradient(to top, rgb(68, 1, 84), rgb(59, 82, 139), rgb(33, 144, 141), rgb(93, 201, 99), rgb(253, 231, 37))";
