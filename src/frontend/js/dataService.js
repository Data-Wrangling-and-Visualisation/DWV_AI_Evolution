...
function transformModelInfo(rawData) {
    ...
    return modelIds.map(id => ({
        id: id, // Keep the ID if needed later
...
export async function getModelData() {
    ...
    if (!modelDataPromise) {
        modelDataPromise = fetch('/api/model-info')
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status} for /api/model-info`);
                }
            ...
            .then(rawData => {
                ...
                console.log("Transformed and cached model data:", cachedModelData.length, "models");
                return cachedModelData;
            })
            .catch(error => {
                console.error('Error fetching or processing model data:', error);
                modelDataPromise = null; // Allow retrying later if needed
    ...
    if (cachedOrgData) {
     ...
     if (!orgDataPromise) {
        orgDataPromise = fetch('/api/organizations-info')
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status} for /api/organizations-info`);
                }
                return response.json();
            })
            .then(data => {
                console.log("Fetched and cached organization data:", data.length, "organizations");
                cachedOrgData = data;
                return cachedOrgData;
            })
            .catch(error => {
                console.error('Error fetching or processing organization data:', error);
                 orgDataPromise = null; // Allow retrying later if needed
                throw error; // Re-throw error to propagate it to the caller
            });
    }
